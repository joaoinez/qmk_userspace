...
flow_state_t flow_state[FLOW_COUNT] = {[0 ... FLOW_COUNT - 1] =
                                           flow_up_unqueued};
...
uint16_t flow_timeout_wait_timers_value[FLOW_COUNT] = {[0 ... FLOW_COUNT - 1] =
                                                           0};

flow_state_t flow_layers_state[FLOW_LAYERS_COUNT] = {
    [0 ... FLOW_LAYERS_COUNT - 1] = flow_up_unqueued};
bool flow_layer_timeout_timers_active[FLOW_LAYERS_COUNT] = {
    [0 ... FLOW_LAYERS_COUNT - 1] = false};
uint16_t flow_layer_timeout_timers_value[FLOW_LAYERS_COUNT] = {
    [0 ... FLOW_LAYERS_COUNT - 1] = 0};
uint16_t flow_layer_timeout_wait_timers_value[FLOW_LAYERS_COUNT] = {
    [0 ... FLOW_LAYERS_COUNT - 1] = 0};
...
bool is_flow_ignored_key(uint16_t keycode) {
    ...
    if (keycode == KC_LSFT || keycode == KC_RSFT || keycode == KC_LCTL ||
        keycode == KC_RCTL || keycode == KC_LALT || keycode == KC_RALT ||
...
bool update_flow_mods(uint16_t keycode, bool pressed) {
    bool pass                                = true;
            ...
            if (flow_key_list_pressed[i]) {
                ...
                switch (flow_state[i]) {
                    case flow_down_unused:
                        if (!flow_pressed[i][1]) {
                            if (timer_elapsed(
                                    flow_timeout_wait_timers_value[i]) >
                                g_flow_oneshot_wait_term) {
                                flow_state[i] = flow_up_unqueued;
                                unregister_code(flow_config[i][1]);
                            } else {
                                // If we didn't use the mod while trigger was
                                // held, queue it.
                        ...
                        break;
                    case flow_down_used:
                        // If we did use the mod while trigger was held,
                        // unregister it.
                        if (!flow_pressed[i][1]) {
                            flow_state[i] = flow_up_unqueued;
                            unregister_code(flow_config[i][1]);
...
bool update_flow_layers(uint16_t keycode, bool pressed, keypos_t key_position) {
        ...
        if (keycode == trigger) {
            if (pressed) {
                ...
                switch (flow_layers_state[i]) {
                    case flow_down_unused:
                        if (timer_elapsed(
                                flow_layer_timeout_wait_timers_value[i]) >
                            g_flow_oneshot_wait_term) {
                            ...
                            // If we didn't use the layer while trigger was
                            // held, queue it.
                        ...
                        break;
                    case flow_down_used:
                        // If we did use the layer while trigger was held, turn
                        // off it.
                        flow_layers_state[i] = flow_up_unqueued;
                        layer_off(layer);
                        change_pressed_status(trigger, false);
...
bool update_flow(uint16_t keycode, bool pressed, keypos_t key_position) {
...
void flow_matrix_scan(void) {
    ...
    for (int i = 0; i < FLOW_LAYERS_COUNT; i++) {
        if (flow_layer_timeout_timers_active[i] &&
            timer_elapsed(flow_layer_timeout_timers_value[i]) >
                g_flow_oneshot_term) {
            flow_layer_timeout_timers_active[i] = false;
            flow_layers_state[i]                = flow_up_unqueued;
